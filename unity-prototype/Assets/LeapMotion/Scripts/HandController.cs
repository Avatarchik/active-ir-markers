...
public class HandController : MonoBehaviour {
  ...
  protected const float MIN_CONFIDENCE = 0.2f;
  protected const float PINCH_THERESHOLD = 0.6f;
  protected const float PINCH_TIMEOUT = 0.2f;
  protected const bool PINCH_ENABLED = false;
  ...
  protected HandModel CreateHand(HandModel model) {
   ...
   // hand_model.gameObject.SetActive(false);
    ...
    Leap.Utils.IgnoreCollisions(hand_model.gameObject, gameObject, true);
    return hand_model;
  }
  ...
  protected void UpdateHandModels(Dictionary<int, HandModel> all_hands,
    ...
    for (int h = 0; h < num_hands; ++h) {
      ...
      //Confidence of Hands
      if(leap_hand.Confidence < MIN_CONFIDENCE)
      {
          continue;
      }
  ...
  protected ToolModel CreateTool(ToolModel model) {
    ...
    Leap.Utils.IgnoreCollisions(tool_model.gameObject, gameObject, true);
    return tool_model;
  }
  ...
  public Frame  GetFrame(int history)
  {

      return leap_controller_.Frame(history);
  }
  ...
  void FixedUpdate() {
    ...
    if (frame.Id != prev_physics_id_)
    {
		...
		{
            if (hand_graphics_.ContainsKey(hand.Id))
			{
				//GameObject.Find("DebugText").GetComponent<Text>().text = hand.PinchStrength.ToString();
                if (hand.PinchStrength > PINCH_THERESHOLD && canPinch && PINCH_ENABLED)
                {
                    pinchTimeout += Time.deltaTime;

                    if (!ballAdded && pinchTimeout > PINCH_TIMEOUT)
                    {
                        HandModel handModel = hand_graphics_[hand.Id];
                        
                        Vector3 position = 0.5f * (handModel.fingers[0].GetTipPosition() +
                                              handModel.fingers[1].GetTipPosition());

                        GameObject[] balls = GameObject.FindGameObjectsWithTag("pinch");
                        foreach (GameObject prev in balls)
                        {
                            Destroy(prev);
                        }
                        ...
                        ball.GetComponent<Renderer>().material = Resources.Load("Def", typeof(Material)) as Material;
                        ball.transform.localScale = new Vector3(scale, scale, scale);                        
                        ball.transform.position = position;
                        ball.GetComponent<SphereCollider>().enabled = false;
                        ball.tag = "pinch";
                        //ball.AddComponent(typeof(SphereCollider));
