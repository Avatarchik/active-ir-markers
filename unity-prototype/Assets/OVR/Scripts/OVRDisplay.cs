...
public class OVRDisplay
{
	...
	{
		get {
            if (!OVRManager.isSingleCamMode)
            {
                return true;
            }

#if !UNITY_ANDROID || UNITY_EDITOR
			return (OVRManager.capiHmd.GetTrackingState(0d).StatusFlags & (uint)StatusBits.HmdConnected) != 0;
#else
    ...
    /// Gets the head pose at the current time or predicted at the given time.
    /// </summary>
    public OVRPose GetCameraPose(double predictionTime /*= 0d*/)
    {
#if !UNITY_ANDROID || UNITY_EDITOR
        double abs_time_plus_pred = Hmd.GetTimeInSeconds() + predictionTime;
        ...
        return state.CameraPose.ToPose(true);
#else
		float px = 0, py = 0, pz = 0, ow = 0, ox = 0, oy = 0, oz = 0;
	...
	public Vector3 acceleration
	{
		get {
#if !UNITY_ANDROID || UNITY_EDITOR
			return OVRManager.capiHmd.GetTrackingState(0d).HeadPose.LinearAcceleration.ToVector3();
#else
	...
	public Vector3 angularVelocity
	{
		get {
#if !UNITY_ANDROID || UNITY_EDITOR
			return OVRManager.capiHmd.GetTrackingState(0d).HeadPose.AngularVelocity.ToVector3();
#else
			float x = 0.0f, y = 0.0f, z = 0.0f;
