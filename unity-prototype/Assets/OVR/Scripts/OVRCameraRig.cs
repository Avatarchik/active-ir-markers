...
{
    private CamCom _camCom = null;

    private Vector3 _camLastPosition = new Vector3();
    private Quaternion _camLastOrientation = new Quaternion();
    ...
    //With IR camera
    //public const float RESET_MIN_MOVEMEMNT = 2.5e-06f;
    //public const float RESET_MIN_ORIENTATION = 0.8f;

    //No camera tracking mode
    public const float RESET_MIN_MOVEMEMNT = 1.5e-04f;
    public const float RESET_MIN_ORIENTATION = 2.4f;


    public const bool DEBUG_ORIENTATION_TRACKING = false;
...
#region Unity Messages
	...
	private void UpdateAnchors()
	{
        ...
        float deltaMovement = (_camLastPosition - centerEyeAnchor.localPosition).sqrMagnitude;
        float deltaOrientation = Quaternion.Angle(_camLastOrientation, centerEyeAnchor.localRotation);

        if (Time.time - _debugTime1 > 0.1f)
        {
            _debugTime1 = Time.time;
            //Debug.Log(string.Format("{0}, {1}", deltaMovement, deltaOrientation));
        }

        if( deltaMovement > RESET_MIN_MOVEMEMNT ||
            deltaOrientation > RESET_MIN_ORIENTATION )
        {
            _camLastPosition = centerEyeAnchor.localPosition;
            _camLastOrientation = centerEyeAnchor.localRotation;

            if (DEBUG_ORIENTATION_TRACKING)
            {
            ...
            //_textInfo3d.text = string.Format("Move: {0}   {1}", deltaMovement, deltaOrientation);  
	...
	private void UpdateCameras()
	{
		...
		{
            if (!OVRManager.isSingleCamMode || OVRManager.display.isPresent)
			{				
			...
			{
				GameObject.Find("QuadRight").SetActive(false);
